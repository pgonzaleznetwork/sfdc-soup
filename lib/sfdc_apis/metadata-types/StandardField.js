...
async function findReferences(connection,entryPoint,cache,options){
    ...
    async function searchFieldInCode(params){
        ...
        console.log(`searching for ${entryPoint.name} in ${metadataType}`);
            ...
            if(body){
                ...
                if(body.match(fieldNameRegex)){
                    ...
                    if(metadataType == 'ApexClass'){
                        ...
                        try {
                            let response = await getStandardFields(sfStandardFieldOptions, {fields: [entryPoint.name]});
                            console.log(`Does the ${member.type} ${member.name} have a reference to ${entryPoint.name}? : `,response);
                        } catch (error) {
                            console.log('error from npm package');
                            //if the above package fails, we assume that the metadata is using the field because 
                            //we found its name through a regular expression 
            ...
            if(body){
                ...
                if(body.match(fieldNameRegex)){
                    ...
                    if(metadataType == 'ApexClass'){
                        ...
                        let response = await getStandardFields(sfStandardFieldOptions, {fields: [entryPoint.name]});
                        console.log(`Does ${member} use the ${entryPoint.name}: `,response);
                    }
                    else{
                        metadataUsingField.push(member);
                    }         
