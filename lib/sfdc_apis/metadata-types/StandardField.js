            ...
            await Promise.all(
                ...
                rawResults.records.map(async record => {
                    ...
                    if(record.SymbolTable && record.Body){
                        ...
                        try {
                            let mapResponse = await getStandardFields(sfStandardFieldOptions, {fields: [entryPoint.name]});

                            let isUsed = mapResponse.has(entryPoint.name);
                            ...
                            //do nothing, this is a best-effort functionality
                        }
                    }
                })
            )
